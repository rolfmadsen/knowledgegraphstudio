...
export function ModelExplorer({ onCollapse }: ModelExplorerProps) {
  const {
    concepts,
    views,
    activeViewId,
  ...
  const groups = concepts.reduce((acc, concept) => {
    const type = getVirtualType(concept, views);
    ...
    <div id="model-explorer-root" className="flex-1 flex flex-col min-h-0 bg-slate-50 border-r border-slate-200">
      ...
      <div className="px-6 py-4 border-b border-slate-200 shrink-0 flex items-center justify-between bg-white shadow-sm z-10">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={onCollapse}
            className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-emerald-600 transition-transform rounded-xl active:scale-95 shrink-0 cursor-pointer"
            title="Skjul Model Explorer (Ctrl+B)"
          >
