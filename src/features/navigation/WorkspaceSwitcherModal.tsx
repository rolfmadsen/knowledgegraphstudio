...
export function WorkspaceSwitcherModal({ isOpen, onClose }: WorkspaceSwitcherModalProps) {
  ...
  const [linkedHandles, setLinkedHandles] = useState<Record<string, { isLinked: boolean; isGranted: boolean }>>({});
  ...
  const handleGrantPermission = async (e: React.MouseEvent, workspacePath: string) => {
    ...
    try {
      if (workspacePath === REPO_DIR) {
        const granted = await FileSystemAccessService.requestActiveHandlePermission();
        if (granted) {
          setIsLoading(true);
          try {
            await switchWorkspace(workspacePath);
        ...
        const handle = await FileSystemAccessService.loadHandleForWorkspace(workspacePath);
        if (handle) {
          const granted = await FileSystemAccessService.verifyPermission(handle, true);
          if (granted) {
            await FileSystemAccessService.setActiveHandle(handle, workspacePath);
